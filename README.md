...
Work on Hierarchical Metalearning for Time Series Forecasting
...
```bash
$ make bash
$ python hmlts/scripts/etl.py configs/examples/config_example.yml
```
...
```bash
python hmlts/scripts/evaluate.py configs/examples/config_example.yml
```
...
└── hmlts    <- Project source directory
